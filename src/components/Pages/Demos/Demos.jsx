  ...
  function renderMatchesElements(matches) {
    return matches.length >= 1 ? (
      <div className="flex items-center justify-center mt-12">
        <div className="w-full lg:w-1/3 col-span-2 pb-5 pl-7 pr-7 pt-7 text-white bg-opacity-75 text-center rounded-lg bg-gray-950 backdrop-blur">
          ...
          {matches.map((match, index) => {
            ...
            return (
              <a
                key={index}
                href={`https://legacystrike.net/demos/${match.demoFileName}`}
                target="_blank"
                rel="noreferrer"
              >
                <div
                ...
                >
                  ...
                  {playedMap ? (
                    <span className="z-10 relative text-white">
                      {playedMap.name} - {moment(match.parsingTime).fromNow()}
                    </span>
                  ) : (
                    <span className="z-10 relative text-white">
                      No mapname found - {moment(matches.parsingTime).fromNow()}
                    </span>
                  )}
      ...
      <div className="flex items-center justify-center mt-20">
        <div className="col-span-2 p-7 text-white bg-opacity-75 text-center rounded-lg bg-gray-950 backdrop-blur">
          <p className="text-lg">
            It seems like something went wrong when gathering demos. Please
