...
export default function Home() {
  ...
  return (
    <div className="w-screen p-12 flex flex-col gap-7 items-center">
      <div style={bgStyle}
        className="bg-black flex flex-col gap-7 bg-opacity-40 p-7 w-full h-full grow rounded-xl">
        <div className="text-white rounded-xl text-center flex flex-col gap-20 p-7 bg-white bg-opacity-5" style={{ backdropFilter: `blur(2px)` }}>
          <div className="flex flex-col gap-3" style={{ height: 200 }}>
            <div className="text-2xl font-semibold">{texts[tabIndex].title}</div>
            <hr />
          ...
          <div className="flex flex-row grow justify-around rounded-xl bg-black bg-opacity-25">
            {texts.map((t, i) =>
              i > 0 &&
              <button key={i} className="grow p-5"
      ...
      <div style={{ maxWidth: 700 }} className="rounded-xl overflow-hidden">
        {discord !== null &&
          <>
            <div className="px-3 py-2 text-white font-bold text-xl flex flex-row justify-between items-center"
            ...
            <div className="flex flex-row flex-wrap gap-2 p-3"
              style={{ backgroundColor: "#2C2F33" }}>
              {
                discord.members.map(m =>
                  <img key={m.id} className="rounded-full" name={m.username} src={m.avatar_url} style={{ height: 24, width: 24 }} />
                )
              }
              <div style={{ backgroundColor: '#23272A' }}
                className="rounded-full text-white px-2">+{discord.presence_count} more online...</div>
            </div>
          </>
        }
      </div>
  ...
  function useDiscord() {
    ...
    async function getDiscord() {
      try {
        const res = await fetch(`https://discord.com/api/guilds/973441288584192061/widget.json`);
